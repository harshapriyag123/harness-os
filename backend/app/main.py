...
app=FastAPI(title='Harness OS API',version='1.0.0');app.add_middleware(CORSMiddleware,allow_origins=['http://localhost:5173'],allow_methods=['*'],allow_headers=['*'])
def required(kind,item_id):
 ...
 return item
def fail(exc):raise HTTPException(404 if isinstance(exc,KeyError) else 409 if isinstance(exc,ValueError) else 503,detail={'code':exc.__class__.__name__.upper(),'message':str(exc).strip("'")})
@app.get('/health')
...
async def campaign_events(cid:str,request:Request,after:int=0):
 ...
 async def stream():
  ...
  while not await request.is_disconnected():
   ...
   for event in batch:cursor=event['sequence'];yield f"id: {cursor}\nevent: {event['event_type']}\ndata: {json.dumps(event)}\n\n"
   campaign=store.get('campaigns',cid)
   if campaign and campaign['status'] in {'COMPLETED','CANCELLED'} and not batch:break
...
def approval_decision(approval_id:str,action:str,body:Decision):
 ...
 raise HTTPException(501,detail={'code':'TRUEFORGE_APPROVAL_NOT_IMPLEMENTED','message':'P3 must resolve the real TrueForge required action; local approval simulation is disabled.'})
...
def integrations():
 try:TrueForgeClient.from_env().capabilities();tf_status='CONNECTED';tf_detail='Official TrueForge HTTP API reachable'
 except TrueForgeError as exc:tf_status='ERROR';tf_detail=str(exc)
 return {'mode':engine.MODE,'integrations':[{'name':'TrueForge','status':tf_status,'detail':tf_detail},{'name':'GitHub MCP','status':'NOT CONFIGURED','detail':'Configure inside TrueForge; P3 writes remain disabled'},{'name':'Chaos MCP','status':'CONFIGURED','detail':'Fixture-only endpoint from HARNESS_CHAOS_MCP_URL'},{'name':'Model Provider','status':'TRUEFORGE MANAGED','detail':'Configure and validate inside TrueForge'}]}
