...
def _github_write_calls(event):
    calls=event.get("tool_calls") or event.get("toolCalls") or []; result=[]
    for call in calls:
        if isinstance(call,dict) and _tool_identifier(call) in GITHUB_WRITE_TOOLS: result.append(call)
    return result
...
def _approved_targets(calls):
    ...
    for call in calls:
        ...
        targets.append({"tool_call_id":str(call.get("id") or call.get("tool_call_id") or call.get("toolCallId")),"tool":_tool_identifier(call),"repository":args.get("repository_full_name") or args.get("repository") or args.get("repo_full_name"),"branch":args.get("branch") or args.get("branch_name") or args.get("head"),"commit_sha":args.get("commit_sha") or args.get("sha"),"pr_number":args.get("pr_number")})
    return targets

def _approval_from_event(campaign,turn_id,event):
    calls=_github_write_calls(event)
    if not calls:return None
    targets=[x for x in _approved_targets(calls) if x["tool_call_id"] and x["tool"]]
