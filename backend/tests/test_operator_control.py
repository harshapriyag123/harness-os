...
class FakeClient:
    def __init__(self,base_url='https://trueforge.example',token='secret',timeout=60):self.base_url=base_url;self.token=token;self.timeout=timeout
    @classmethod
    def from_env(cls):return cls()
    def capabilities(self):raise TrueForgeError('Invalid/timeout response from TrueForge: timed out')
...
class OperatorControlTest(unittest.TestCase):
    ...
    def test_snapshot_prioritizes_waiting_approval(self,list_records,get_record,_events):
        ...
        def records(kind):
            ...
            return []
        list_records.side_effect=records;get_record.return_value={'id':'a1','name':'repo'}
        result=operator_control.snapshot()
        self.assertEqual('c-wait',result['campaign']['id']);self.assertEqual('apr1',result['approval']['id']);self.assertEqual('WAITING_APPROVAL_PRIORITY',result['selection_reason'])
    ...
    def test_explicit_target_does_not_fall_back_to_other_campaign(self,list_records,get_record):
        ...
        self.assertIsNone(result['campaign']);self.assertEqual('a1',result['target']['id']);self.assertEqual('NO_CAMPAIGN_FOR_TARGET',result['selection_reason'])
    ...
    def test_trueforge_timeout_is_actionable(self):
        ...
        self.assertEqual('TIMEOUT',result['status']);self.assertTrue(result['retryable']);self.assertIn('TRUEFORGE_BASE_URL',result['diagnosis']);self.assertNotIn('secret',str(result))
    ...
    def test_invalid_probe_timeout_falls_back_instead_of_crashing(self):
        result=operator_control.trueforge_status()
        self.assertEqual(4.0,result['probe_timeout_seconds']);self.assertIn('Invalid TRUEFORGE_PROBE_TIMEOUT_SECONDS',result['configuration_warning'])
