...
const TRUEFORGE=(import.meta.env.VITE_TRUEFORGE_PUBLIC_URL||'https://harness-os-trueforge.onrender.com').replace(/\/$/,'');
...
export default function PublicMissionControl(){
 ...
 const links=useMemo(()=>[
  ['TRUEFORGE','HOSTED',TRUEFORGE,'Dockerized agent harness on Render'],
  ['HARNESS API','LIVE',API,'Cloud control-plane API'],
 ...
 return <main className="mc-shell pmc-shell">
  <header className="mc-header"><div className="mc-brand"><ShieldCheck/><div><strong>Harness OS</strong><span>Agent Safety Mission Control · Public Judge Runtime</span></div></div><div className="mc-header-right"><span className="mc-runtime ok"><i/>HOSTED TrueForge</span><span className="mc-runtime ok"><Globe2/>public evidence stack</span></div></header>

  <section className="pmc-hero">
   <div><span>TRUEFORGE AGENT HARNESS HACKATHON</span><h1>Crash-test autonomous agents before production.</h1><p>TrueForge owns the agent loop, deferred MCP tools, session runtime and approval boundary. Harness OS projects the evidence chain into a judge-readable control plane.</p></div>
   <div className="pmc-actions"><button className="mc-primary" onClick={()=>open(TRUEFORGE)}><Play/>Open hosted TrueForge</button><button className="mc-secondary" onClick={()=>open(API)}><Server/>Cloud API</button><button className="mc-secondary" onClick={()=>open(ARCH)}><ExternalLink/>Architecture</button></div>
  </section>
