...
export default function JudgeDemoCore(){
 const[open,setOpen]=useState(false),[ollama,setOllama]=useState<any>({status:'CHECKING'}),[tf,setTf]=useState<any>({status:'CHECKING'}),[services,setServices]=useState<any[]>([]),[error,setError]=useState(''),[checking,setChecking]=useState(false),[copied,setCopied]=useState('');
 async function refresh(){setChecking(true);setError('');try{const[trueforge,publics]=await Promise.all([api.trueforgeStatus(),api.publicServices(true)]);setTf(trueforge);setServices(publics.services||[]);try{const r=await fetch(`${OLLAMA}/api/tags`);if(!r.ok)throw new Error(`HTTP ${r.status}`);const body=await r.json();const models=(body.models||[]).map((m:any)=>m.name||m.model).filter(Boolean);setOllama({status:'CONNECTED',models,model:models[0]||'No model pulled'})}catch(e:any){setOllama({status:'UNAVAILABLE',model:'—',detail:e.message})}}catch(e:any){setError(e.message)}finally{setChecking(false)}}
 useEffect(()=>{refresh()},[]);
 ...
 return <aside className={`judge-core ${open?'open':''}`}>
  ...
  {open&&<div className="judge-core-panel">
   ...
   <section className="judge-core-readiness">
    <article className={ollama.status==='CONNECTED'?'ok':'bad'}><Cpu/><div><span>MODEL</span><strong>Ollama {ollama.status}</strong><small>{ollama.model||'No model detected'}</small></div></article>
   ...
   <section className="judge-core-cloud"><Cloud/><div><span>PUBLIC HACKATHON SURFACE</span><strong>GitHub Pages + Render services</strong><p>Public judge page is read-only. Local TrueForge remains the consequential-action runtime; cloud deployment manifests are included for container hosting.</p></div></section>
