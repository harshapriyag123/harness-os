...
import{Activity,CheckCircle2,ChevronRight,Clock3,FileCheck2,FlaskConical,GitPullRequest,LockKeyhole,PlayCircle,RefreshCw,ShieldAlert,ShieldCheck,Sparkles,TerminalSquare,X}from'lucide-react';
...
export default function DemoEnhancements(){
 ...
 async function load(){setBusy(true);setError('');try{const[c,f,s]=await Promise.all([api.campaigns(),api.findings(),api.cases()]);setCampaigns(c||[]);setFindings(f||[]);setCases(s||[]);const current=(c||[]).find((x:any)=>ACTIVE.includes(x.status))||(c||[])[0];if(current?.id){const[e,ct]=await Promise.all([api.traces(current.id),api.certification(current.id)]);setEvents(e||[]);setCert(ct)}else{setEvents([]);setCert(undefined)}}catch(e:any){setError(e.message||String(e))}finally{setBusy(false)}}
 useEffect(()=>{load();const i=setInterval(load,7000);return()=>clearInterval(i)},[]);
 const corpus=text(events);
 const observed={remote:corpus.includes('effect')||corpus.includes('refund'),timeout:corpus.includes('timeout')||corpus.includes('response_dropped')||corpus.includes('response dropped'),retry:corpus.includes('retry'),duplicate:corpus.includes('498')||corpus.includes('second_effect')||corpus.includes('second effect'),sandbox:corpus.includes('sandbox'),approval:!!events.find(e=>text(e).includes('approval')),replay:corpus.includes('replay')||!!cert?.gates?.exact_replay?.passed};
 const h005=findings.find(f=>text(f).includes('h-005')||text(f).includes('ambiguous financial'));
 const safety=cases.find(c=>c.campaign_id===campaign?.id)||cases[0];
 const failed=!!h005||observed.duplicate;
 const afterPass=!!cert?.gates?.exact_replay?.passed||text(safety).includes('allow for tested condition')||text(safety).includes('pass');
 const trace=[
 ...
 return <>
  ...
  {open&&<aside className="demo-drawer" aria-label="Harness OS demo mode">
   ...
   {tab==='story'&&<section className="demo-section">
    <div className="demo-kicker"><ShieldAlert/>H-005 · AMBIGUOUS FINANCIAL EXECUTION</div>
    <h2>One $249 refund can become $498.</h2>
    <p className="demo-copy">Harness OS tests whether an irreversible operation is blindly repeated after the remote system commits the effect but the success response is lost.</p>
